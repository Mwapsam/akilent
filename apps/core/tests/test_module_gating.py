"""Phase 0: a disabled module cannot execute - on views, workflow enrollment and actions.

Semantics (backward compatible): only an explicit ModuleSubscription(enabled=False) row
blocks a module. A tenant with no row keeps today's behaviour (allowed), so deploying
...
def set_module(account, module, enabled):
    ModuleSubscription.objects.update_or_create(account=account, module=module,
...
def test_module_enabled_semantics(logged_in):
    _, account = logged_in
    assert billing_api.module_enabled(account, "crm") is True        # no row -> allowed
    set_module(account, "crm", False)
    assert billing_api.module_enabled(account, "crm") is False       # explicit disable blocks
    set_module(account, "crm", True)
    assert billing_api.module_enabled(account, "crm") is True
    with pytest.raises(KeyError):
        billing_api.module_enabled(account, "not-a-module")
...
def test_disabling_one_tenant_does_not_affect_another(logged_in):
    ...
    set_module(account, "crm", False)
    assert billing_api.module_enabled(other, "crm") is True


# ---- views ----------------------------------------------------------------------
@pytest.mark.django_db
@pytest.mark.parametrize("url,module", [("/sales/", "crm"), ("/orders/", "commerce"),
...
def test_view_allowed_by_default_blocked_when_disabled(logged_in, url, module):
    ...
    resp = client.get(url)
    assert resp.status_code == 302 and resp.url == "/dashboard/"      # redirected to the dashboard
...
def test_disabled_automation_module_blocks_enrollment(logged_in, contact):
    ...
    set_module(account, "automation", False)
...
def test_enrollment_still_works_with_no_row_or_enabled(logged_in, contact):  # regression
    ...
    set_module(account, "automation", True)
    assert enroll(wf, contact) is not None
...
def test_disabled_crm_module_blocks_crm_actions(logged_in, contact):
    _, account = logged_in
    set_module(account, "crm", False)
    with pytest.raises(ActionError, match="crm"):
...
def test_disabled_commerce_module_blocks_commerce_actions(logged_in, contact):
    _, account = logged_in
    set_module(account, "commerce", False)
    with pytest.raises(ActionError, match="commerce"):
...
def test_modules_gate_independently(logged_in, contact):
    _, account = logged_in
    set_module(account, "commerce", False)          # commerce off must not block crm
    assert "lead_id" in run_action("create_lead", {"account": account}, account=account, contact=contact)
...
def test_workflow_api_blocked_when_automation_disabled(client, db):
    ...
    set_module(acc, "automation", False)
